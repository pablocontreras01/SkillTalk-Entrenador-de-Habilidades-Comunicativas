...
COLORS = {
"Beat": (0, 255, 0),
"No-Gesture": (255, 165, 0)
}

# Constantes de esqueletos
...
shoulder_vec = np.array([1.0, 0.0, 0.0]) if norm < 1e-6 else shoulder_vec / norm

target = np.array([1.0, 0.0, 0.0])
v = np.cross(shoulder_vec, target)
c = np.dot(shoulder_vec, target)
R = np.eye(3) if np.linalg.norm(v) < 1e-6 else np.eye(3) + np.array([[0,-v[2],v[1]],[v[2],0,-v[0]],[-v[1],v[0],0]]) + np.array([[0,-v[2],v[1]],[v[2],0,-v[0]],[-v[1],v[0],0]]) @ np.array([[0,-v[2],v[1]],[v[2],0,-v[0]],[-v[1],v[0],0]]) * (1 / (1 + c))
seq = seq @ R.T
...
k[20] = spine_shoulder
k[21] = L(mp_pose.PoseLandmark.LEFT_INDEX); k[22] = L(mp_pose.PoseLandmark.LEFT_THUMB)
k[23] = L(mp_pose.PoseLandmark.RIGHT_INDEX); k[24] = L(mp_pose.PoseLandmark.RIGHT_THUMB)
return k
...
# 💾 FUNCIONES DE VIDEO Y VISUALIZACIÓN
...
current_k25 = last_valid_k25.copy() if (last_valid_k25 is not None and repeat_last_valid) else np.zeros((25,3), dtype=np.float32)
frame_data.append({'frame': frame_bgr, 'k25': current_k25, 'pose_landmarks': current_pose_landmarks})
cap.release()
return frame_data
...
@st.cache_resource
def load_ml_model(model_filename: str):
...
st.error(f"❌ Error al cargar el modelo desde {model_path}. Detalle: {e}")
return None
...
with st.spinner('Paso 1/3: Extrayendo esqueletos del video...'):
try:
...
```
MODEL_FILENAME = "mlp_lstm_ted.h5"
...
if uploaded_file is not None:
    ...
    with col_result:
        if st.button("▶️ Ejecutar Clasificación"):
            st.info("Procesando video... puede tardar varios minutos.")
